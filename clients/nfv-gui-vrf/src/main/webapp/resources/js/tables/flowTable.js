...
//        headers[3] = "In";
        ...
        if (isStringArray) {
            ...
            if (headers) {
		var regex = new RegExp("10.0.[0-9].9[0-9]");
                for (i = 0; i < arr_size; i++) {
                    if(parsedJson.floodlightOFFlows.floodlightOFFlow[i].actions.value != -2){
			if(parsedJson.floodlightOFFlows.floodlightOFFlow[i].match.srcIp){
			if(!parsedJson.floodlightOFFlows.floodlightOFFlow[i].match.srcIp.match(regex)){
